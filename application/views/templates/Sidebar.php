 <aside class="main-sidebar">
  <!-- sidebar: style can be found in sidebar.less -->
  <section class="sidebar">
    <!-- Sidebar user panel -->
    <div class="user-panel">
      <div class="pull-left image">
        <img src="<?php echo base_url() ?>assets/template/back/dist/img/user2-160x160.jpg" class="img-circle" alt="User Image">
      </div>
      <div class="pull-left info">
        <p><?= $user['nama']; ?></p>
        <a href="#"><i class="fa fa-circle text-success"></i> Online</a>
      </div>
    </div>
    <!-- search form -->
      <!-- <form action="#" method="get" class="sidebar-form">
        <div class="input-group">
          <input type="text" name="q" class="form-control" placeholder="Search...">
          <span class="input-group-btn">
                <button type="submit" name="search" id="search-btn" class="btn btn-flat"><i class="fa fa-search"></i>
                </button>
              </span>
        </div>
      </form> -->
      <!-- /.search form -->
      <!-- sidebar menu: : style can be found in sidebar.less -->
      <ul class="sidebar-menu" data-widget="tree">
        <li class="header">MENU ADMINISTRATOR</li>

      <li><a href="<?php echo base_url('admin/dasbor'); ?>"><i class="fa fa-check text-aqua"></i>
          <span>Dashboard</span></a></li>
          
        </li>
        <li class="menu treeview">
          <a href="<?php echo base_url('admin/user'); ?>">
            <i class="fa fa-user"></i> <span>Data Users</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
      <ul class="treeview-menu">
            <li class="active"><a href="<?php echo base_url('admin'); ?>"><i class="fa fa-circle-o"></i> Data Admin</a></li>
            <li class="active"><a href="<?php echo base_url('penjual'); ?>"><i class="fa fa-circle-o"></i> Data Penjual</a></li>
            <li class="active"><a href="<?php echo base_url('pelanggan'); ?>"><i class="fa fa-circle-o"></i> Data Pelanggan</a></li>
             <li class="active"><a href="<?php echo base_url('karyawan'); ?>"><i class="fa fa-circle-o"></i> Data Karyawan</a></li>
          </ul>
      </li>

        <li class="menu treeview">
          <a href="<?php echo base_url('admin/produk'); ?>">
            <i class="fa fa-sitemap"></i> <span>Data Produk</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
            <li class="active"><a href="<?php echo base_url('produkA'); ?>"><i class="fa fa-circle-o"></i> Produk Warungkoe</a></li>
          </ul>
        </li>
          <li class="menu treeview">
          <a href="<?php echo base_url('admin/produk'); ?>">
            <i class="fa fa-floppy-o"></i> <span>Testimoni</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
            <li class="active"><a href="<?php echo base_url('RatingA'); ?>"><i class="fa fa-circle-o"></i>Data Testimoni</a></li>
          </ul>
        </li>

  </section>
  <!-- /.sidebar -->
</aside>